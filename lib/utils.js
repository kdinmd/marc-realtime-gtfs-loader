...
const getTimeout = (offsetInMilliseconds, desiredTimeoutInSeconds) => {
  if (offsetInMilliseconds < 0) {
    throw new Error('getTimeout: Offset must be non-negative');
  }

  const timeoutRemaining = secondsToMilliseconds(desiredTimeoutInSeconds) - offsetInMilliseconds;
