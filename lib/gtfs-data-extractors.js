...
const getStopTimeUpdates = (trips) => {
  const stopTimes = [];
  ...
  trips?.forEach((trip) => {
    ...
    try {
      ...
      console.error('Recieved data without required fields in getStopTimeUpdates');
      errorOccured = true;
    }
  });
...
const getActiveTrips = (trips) => {
  ...
  trips?.forEach((trip) => {
    ...
    try {
      ...
      console.error('Error while creating new Active Trips in getActiveTrips');
      errorOccured = true;
    }
  });
...
const getTrainPositions = (trips) => {
  ...
  trips?.forEach((trip) => {
    ...
    try {
      ...
      console.error('Error while creating new Train Position in getTrainPositions');
      errorOccured = true;
    }
  });
...
module.exports = { getStopTimeUpdates, getActiveTrips, getTrainPositions };
