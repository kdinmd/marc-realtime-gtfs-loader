...
const replace = async (activeTableName, shadowTableName, rows) => {
  try {
    ...
    if (rows && rows.length > 0) {
      ...
      await trx(activeTableName).truncate();
    }
    await trx.commit();

    if (insertionResult && insertionResult.rowCount > 0) {
      console.log(`${insertionResult.rowCount} Rows replaced existing rows in ${activeTableName}`);
    } else {
